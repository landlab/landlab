...
def create_dicts_of_cats():
    ...
    return (all_methods_for_cat_allgrid, all_cats_for_method_allgrid, fails_allgrid)
    ...
    for LLCAT in LLCATS:
        ...
        try:
            ...
            for meth in allmeths:
                if LLCAT in ("GINF", "NINF", "LINF", "CINF", "PINF", "FINF", "CNINF"):
                    exclude = ("DEPR", "MAP", "GRAD", "SURF")
                else:
                    exclude = ("DEPR",)
                if np.in1d(exclude, all_cats_for_method_allgrid[grid][meth]).sum() == 0:
                    text_to_add += (
                        "    ~landlab.grid."
                        + grid_to_modify
                        + "."
