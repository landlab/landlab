...
def test_init_from_points(mesh_from_points):
    ...
    assert_array_equal(
        mesh._segments,
        [[i, i + 1] if i != 11 else [11, 0] for i in range(len(xy_points))],
    )
...
def test_init_from_dims(mesh_from_dims):
    ...
    assert_array_equal(
        mesh._segments, [[i, i + 1] if i != 11 else [11, 0] for i in range(12)]
    )
...
def test_init_from_shapefile(mesh_from_shapefile):
    ...
    assert_array_equal(
        mesh._vertices,
        [[393, -211], [792, -208], [803, -530], [390, -523], [393, -211]],
    )

    assert_array_equal(
        mesh._segments, [[i, i + 1] if i != 3 else [3, 0] for i in range(4)]
    )
