...
def test_framed_voronoi_from_dict():
    ...
    assert mg._xy_spacing == (10., 15.)
    assert mg._xy_of_lower_left == (1.0, 2.0)
    assert mg._xy_min_spacing == (5., 7.5)
    assert mg._orientation == "horizontal"
    assert mg._node_layout == "rect"
    assert mg._random_seed == False
    assert mg._seed == (200, 500)
    assert mg.xy_of_reference == (0.5, 3.0)
    assert mg._axis_name == ("a", "b")
    assert mg._axis_units == ("l", "l")

    true_x = np.array([  1.        ,  11.        ,  21.        ,  31.        ,
        41.        ,  51.        ,   1.        ,  39.38915671,
        11.73417072,  28.64950826,  21.81959986,  51.        ,
         1.        ,  31.13460221,  11.36748325,  40.37121407,
        21.25582009,  51.        ,   1.        ,  20.34996285,
         8.80397049,  30.28757262,  39.70778522,  51.        ,
         1.        ,  11.        ,  21.        ,  31.        ,
        41.        ,  51.        ])
    true_y = np.array([  2.      ,   2.      ,   2.      ,   2.      ,   2.      ,
         2.      ,  13.249   ,  16.333676,  17.500574,  18.090179,
        19.654835,  20.751   ,  28.249   ,  30.593924,  31.76024 ,
        34.146656,  34.410209,  35.751   ,  43.249   ,  45.107227,
        46.516626,  46.566092,  49.977137,  50.751   ,  62.      ,
        62.      ,  62.      ,  62.      ,  62.      ,  62.      ])
    true_adjacent_nodes_at_node = np.array([[ 1,  6, -1, -1, -1, -1, -1],
       [ 2,  8,  6,  0, -1, -1, -1],
       [ 3,  9, 10,  8,  1, -1, -1],
       [ 4,  7,  9,  2, -1, -1, -1],
       [ 5,  7,  3, -1, -1, -1, -1],
