...
class TestGetLinknodes:
    ...
    def test_get_link_nodes_fine(self, nmgrid_f):
        ...
        link_nodes_f_e = np.array([[0, 1],
                                  ...
                                  [5, 6],
                                  [4, 7]])        
        check_vals(link_nodes_f,
                       link_nodes_f_e)
        
    def test_get_link_nodes_slope_is_zero(self, nmgrid_c):
        """what happens if no slope?"""
        nmgrid_c.at_node['topographic__elevation'][1] = 1 # set elevation of second node equal to first, so no slope
        link_nodes_c = gt.get_link_nodes(nmgrid_c)
        link_nodes_c_e = np.array([[-1, -1], # there are no downstream or upstream nodes, both listed as -1
                                [1, 2],
                                [1, 3]])        
...
class TestExtractTerraceNodes:
    ...
    def test_extract_terrace_nodes_2(self, grid):
        ...
        fcn = np.array([ 3,  9, 17, 23])
        tn = gt.extract_terrace_nodes(grid, terrace_width, acn, fcn)
        tn_e = np.array([ 0,  1,  2,  4,  5,  7,  8, 10, 11, 12, 14, 15, 16, 18, 19, 21, 22,
               24, 25, 26, 28, 29, 33, 35, 37])
        check_vals(tn,tn_e)
...
class TestMapNMGLinksToRMGCoincidentNodes:
    ...
    def test_map_nmg_links_to_rmg_coincident_nodes_1(self,grid,nmgrid_c):
        """remove duplicates"""
        link_nodes_c = gt.get_link_nodes(nmgrid_c)
        nmg_link_to_rmg_coincident_nodes_mapper = gt.map_nmg_links_to_rmg_coincident_nodes(grid, 
                                                                                           nmgrid_c, 
                                                                                           link_nodes_c, 
                                                                                           remove_duplicates = True)
        
        nmg_link_to_rmg_coincident_nodes_mapper_e = np.array([[0,                3,  30.0,   0.0,  31.622777,         2300.0],
        ...
        check_vals(nmg_link_to_rmg_coincident_nodes_mapper.iloc[0:5].to_numpy(),nmg_link_to_rmg_coincident_nodes_mapper_e)
        
        # check no duplicate nodes
        assert len(np.unique(nmg_link_to_rmg_coincident_nodes_mapper['coincident_node'].values)) == len(nmg_link_to_rmg_coincident_nodes_mapper['coincident_node'].values)
    ...
    def test_map_nmg_links_to_rmg_coincident_nodes_3(self,grid,nmgrid_f):
        ...
        nmg_link_to_rmg_coincident_nodes_mapper_e = np.array([[0,                3,  30.0,   0.0,  14.142136,         2750.0],
