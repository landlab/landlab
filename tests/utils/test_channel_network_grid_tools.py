...
class TestMapNetworkLinksToNodes:

    def test_map_network_links_to_nodes_1(self, grid, nmgrid_c):
        """coarse-scale network model grid, remove duplicates"""
        link_nodes_c = gt.get_link_nodes(nmgrid_c)
        nmg_link_to_node_mapper = gt.map_network_links_to_nodes(
            grid, nmgrid_c, link_nodes_c, remove_duplicates=True
        )

        nmg_link_to_node_mapper_e = np.array(
            [
        ...
        check_vals(
            np.array(list(nmg_link_to_node_mapper.values())).T,
            nmg_link_to_node_mapper_e,
        )

        # check no duplicate nodes
        assert len(np.unique(nmg_link_to_node_mapper["node"])) == len(
            nmg_link_to_node_mapper["node"]
        )

    def test_map_network_links_to_nodes_2(self, grid, nmgrid_c):
        """coarse-scale network model grid, don't remove duplicate nodes"""
        link_nodes_c = gt.get_link_nodes(nmgrid_c)
        nmg_link_to_node_mapper_dup = gt.map_network_links_to_nodes(
            grid, nmgrid_c, link_nodes_c, remove_duplicates=False
        )

        nmg_link_to_node_mapper_dup_e = np.array(
            [
                [0.0, 3.0, 30.0, 0.0, 31.6227766, 2300.0],
                [0.0, 10.0, 30.0, 10.0, 26.33648662, 2300.0],
                [0.0, 16.0, 20.0, 20.0, 15.79556109, 2300.0],
        ...
        check_vals(
            np.array(list(nmg_link_to_node_mapper_dup.values())).T,
            nmg_link_to_node_mapper_dup_e,
        )

        # check that duplicate nodes are included
        assert len(np.unique(nmg_link_to_node_mapper_dup["node"])) != len(
            nmg_link_to_node_mapper_dup["node"]
        )

    def test_map_network_links_to_nodes_3(self, grid, nmgrid_f):
        ...
        check_vals(
            np.array(list(nmg_link_to_node_mapper.values())).T[:5],
            nmg_link_to_node_mapper_e,
        )
    ...
    def test_map_network_links_to_nodes_4(self, grid_up, nmgrid_c_up):
        """upside down coarse-scale network model grid, remove duplicates"""
        link_nodes_c_up = gt.get_link_nodes(nmgrid_c_up)
        nmg_link_to_node_mapper_up = gt.map_network_links_to_nodes(
            grid_up, nmgrid_c_up, link_nodes_c_up, remove_duplicates=True
        )

        nmg_link_to_node_mapper_up_e = np.array(
            [
        ...
        check_vals(
            np.array(list(nmg_link_to_node_mapper_up.values())).T,
            nmg_link_to_node_mapper_up_e,
        )

        # check no duplicate nodes
        assert len(np.unique(nmg_link_to_node_mapper_up["node"])) == len(
            nmg_link_to_node_mapper_up["node"]
        )

    def test_map_network_links_to_nodes_5(self, grid, nmgrid_s):
        """single-channel-reach network model grid, remove duplicates"""
        link_nodes_s = gt.get_link_nodes(nmgrid_s)
        nmg_link_to_node_mapper_s = gt.map_network_links_to_nodes(
            grid, nmgrid_s, link_nodes_s, remove_duplicates=True
        )

        nmg_link_to_node_mapper_s_e = np.array(
            [
                [0.0, 3.0, 30.0, 0.0, 31.6227766, 2300.0],
                [0.0, 10.0, 30.0, 10.0, 26.33648662, 2300.0],
                [0.0, 16.0, 20.0, 20.0, 15.79556109, 2300.0],
        ...
        check_vals(
            np.array(list(nmg_link_to_node_mapper_s.values())).T,
            nmg_link_to_node_mapper_s_e,
        )

        # check no duplicate nodes
        assert len(np.unique(nmg_link_to_node_mapper_s["node"])) == len(
            nmg_link_to_node_mapper_s["node"]
        )

    def test_map_network_links_to_nodes_6(self, grid_up, nmgrid_s_up):
        """upside down single-channel-reach network model grid, remove duplicates"""
        link_nodes_s_up = gt.get_link_nodes(nmgrid_s_up)
        nmg_link_to_node_mapper_s_up = gt.map_network_links_to_nodes(
            grid_up, nmgrid_s_up, link_nodes_s_up, remove_duplicates=True
        )

        nmg_link_to_node_mapper_s_up_e = np.array(
            [
        ...
        check_vals(
            np.array(list(nmg_link_to_node_mapper_s_up.values())).T,
            nmg_link_to_node_mapper_s_up_e,
        )

        # check no duplicate nodes
        assert len(np.unique(nmg_link_to_node_mapper_s_up["node"])) == len(
            nmg_link_to_node_mapper_s_up["node"]
        )
