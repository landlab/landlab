...
def test_nearest_node_with_one_target_batch():
    find_nearest = NearestNodeFinder([[0.0, 0.0], [2.0, 2.0], [5.0, 5.0]])
    nodes = find_nearest([[1.9, 2.1]])
    assert_array_equal(nodes, [1])


def test_nearest_node_with_no_targets():
    find_nearest = NearestNodeFinder([[0.0, 0.0], [2.0, 2.0], [5.0, 5.0]])
    nodes = find_nearest(np.empty((0, 2)))
    assert nodes.shape == (0,)
    assert np.issubdtype(nodes.dtype, np.integer)


@pytest.mark.parametrize("coords_of_target", ([], [[]]))
def test_nearest_node_with_invalid_empty_target(coords_of_target):
    find_nearest = NearestNodeFinder([[0.0, 0.0], [2.0, 2.0], [5.0, 5.0]])
    with pytest.raises(ValueError, match="coords_of_target"):
        find_nearest(coords_of_target)


def test_nearest_node_in_three_dimensions():
    find_nearest = NearestNodeFinder(
        [[0.0, 0.0, 0.0], [2.0, 2.0, 2.0], [5.0, 5.0, 5.0]]
    )
    assert find_nearest([1.9, 2.1, 2.0]) == 1
    assert_array_equal(find_nearest([[1.9, 2.1, 2.0]]), [1])
