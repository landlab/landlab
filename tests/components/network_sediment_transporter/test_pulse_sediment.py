...
x_of_node = np.arange(0,1000000,100000)
...
element_id = np.repeat(np.arange(grid.size("link")),2)
...
flow_depth =  0.02 * np.ones([timesteps + 1,grid.number_of_links])
...
# JUST ADD ONE NEW TIMESTEP WITH PULSE. 
...
#%%
print(parcels.dataset.element_id.values)
# issue one, this works, need to make example where it doens't work. 

# issue two, element ID must be integer. 
