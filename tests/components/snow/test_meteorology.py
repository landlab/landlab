...
def test_update_sensible_heat_flux():
    ...
    assert_almost_equal(
        ...
        np.array([0.,  69.2466541, -27.6720539, -27.6720539])
    )  # Qh
...
def test_update_vapor_pressure():
    ...
    grid.add_full("land_surface__latitude", 40, at="node")
    grid.add_full("land_surface__longitude", -105, at="node")
    grid.add_full("atmosphere_bottom_air_water-vapor__relative_saturation",
                  0.4, at="node")
    ...
    assert_almost_equal(
        grid.at_node["atmosphere_bottom_air_water-vapor__partial_pressure"],
        0.26280276
    )  # e_air
    ...
    assert_almost_equal(
        grid.at_node["land_surface_air_water-vapor__partial_pressure"],
        0.22714628
    )  # e_surf
...
def test_update_conduction_heat_flux():
    ...
    assert_almost_equal(
        grid.at_node["snowpack_land_surface_net-conduction-heat__energy_flux"],
        0
    )  # Qc
...
def test_update_advection_heat_flux():
    ...
    assert_almost_equal(
        grid.at_node["snowpack_land_surface_net-advection-heat__energy_flux"],
        0
    )  # Qa
...
def test_update_julian_day():  # TODO: double this calculation
    ...
    grid.add_full("land_surface__temperature", -1, at="node")
    grid.add_full("land_surface__latitude", 40, at="node")
    grid.add_field("land_surface__longitude",
                   np.array([-75., -95., -105., -125.]),
                   at="node")
    ...
    assert_almost_equal(
        met._TSN_offset,
        np.array([0.07271961, -0.26061373, 0.07271961, -0.26061373])
    )  # TSN_offset
...
def test_update_net_shortwave_radiation():
    ...
    assert_almost_equal(
        grid.at_node["land_surface_net-shortwave-radiation__energy_flux"],
        537.4501879
    )  # Qn_SW
...
def test_update_em_air():
    ...
    assert_almost_equal(
        grid.at_node["atmosphere_bottom_air__emissivity"],
        0.90170103
    )  # em_air
...
def test_update_net_longwave_radiation():
    ...
    assert_almost_equal(
        grid.at_node["land_surface_net-longwave-radiation__energy_flux"],
        -21.79438345
    )  # Qn_LW
