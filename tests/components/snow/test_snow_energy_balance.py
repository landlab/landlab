...
def test_create_fields():
    ...
    assert_almost_equal(
        ...
        300)
    assert_almost_equal(
        grid.at_node["snowpack__z_mean_of_mass-specific_isobaric_heat_capacity"],
        2090.0)
...
def test_assign_parameters():
    ...
    sm = SnowEnergyBalance(grid, rho_H2O=1003, rho_air=1.2, Cp_air=1004,
                           T_rain_snow=1.2, T0_cc=0.1, grid_area=1000)
...
def test_snow_accumulation():
    ...
    assert_almost_equal(grid.at_node["snowpack__melt_volume_flux"], 0)
    # TODO: may need to change Ecc as 4e6
    assert_almost_equal(grid.at_node["snowpack__energy-per-area_cold_content"], 2e6)
    assert_almost_equal(
...
def test_snow_melt():
    ...
    grid.add_full("atmosphere_bottom_air__temperature", 1, at="node")
    grid.add_full("land_surface__temperature", -1, at="node")
    grid.add_full("land_surface_net-total-energy__energy_flux", 2e3+334000*1000,
                  at="node")  # 334000 energy to melt 0.001 m/s
    grid.add_full("snowpack__liquid-equivalent_depth", 1, at="node")
    grid.add_full("snowpack__z_mean_of_mass-per-volume_density", 200, at='node')
    grid.add_full("snowpack__z_mean_of_mass-specific_isobaric_heat_capacity",
                  2000, at='node')
...
def test_snow_melt_accumulation():
    ...
    assert_almost_equal(grid.at_node["snowpack__melt_volume_flux"], 1/1000)
    # TODO may need to update Ecc > 0 swe !=0
    assert_almost_equal(grid.at_node["snowpack__energy-per-area_cold_content"], 0)
    assert_almost_equal(
...
def test_multiple_steps():
    ...
    grid.add_full("land_surface__temperature", -1, at="node")
    grid.add_full("land_surface_net-total-energy__energy_flux", 2e3+334000*1000,
                  at="node")  # 334000*1000 energy to melt 1 m/s
    grid.add_full("snowpack__liquid-equivalent_depth", 1, at="node")
    ...
    assert_almost_equal(grid.at_node["snowpack__melt_volume_flux"], 0)
    # TODO: may need to change Ecc to 6e6 because swe != 0
    assert_almost_equal(grid.at_node["snowpack__energy-per-area_cold_content"], 0)
    assert_almost_equal(
    ...
    grid.at_node["atmosphere_bottom_air__temperature"].fill(1)
    grid.at_node["atmosphere_water__precipitation_leq-volume_flux"].fill(0.003)
    # TODO: may need to change Q_sum to account for Ecc>0 for step2
    ...
    # TODO: may need to change Ecc>0 because swe != 0
    assert_almost_equal(grid.at_node["snowpack__energy-per-area_cold_content"], 0)
    assert_almost_equal(
