...
def test_simple_water_table_adaptive_dt_ivp():
    """Test a one-node steady simulation.

    Notes
    -----
    This test demonstrates the same simple water table as
    test_simple_water_table, but with the
    run_with_adaptive_time_step_plus_ivp_solver method.
...
def test_simple_surface_leakage_adaptive_dt():
    """test a two-node steady simulation for surface leakage.

    Notes
    ----
    This test demonstrates that at steady state when no flow is
    allowed to leave the domain through the subsurface, the surface
    water flux is equal to the recharge flux. This test uses
    the ``run_with_adaptive_time_step_solver`` method, and thus we use
    two nodes to allow a stability timestep to be calculated.
    ...
    for i in range(100):
        gdp.run_with_adaptive_time_step_solver(1e3)

    assert_almost_equal(gdp._qs[5], 1e-6)


def test_simple_surface_leakage_adaptive_dt_ivp():
    """test a two-node steady simulation for surface leakage.
    ...
    allowed to leave the domain through the subsurface, the surface
    water flux is equal to the recharge flux. This test uses
    the ``run_with_adaptive_time_step_plus_ivp_solver`` method, and thus we use
    two nodes to allow a stability timestep to be calculated.
    ...
    for i in range(100):
        gdp.run_with_adaptive_time_step_plus_ivp_solver(1e3)
...
def test_conservation_of_mass_adaptive_dt_ivp():
    """test conservation of mass in a sloping aquifer.

    Notes
    ----
    Same as ``test_conservation_of_mass_adaptive_dt`` but uses the
    ...
    dt = 1e4
    for i in range(500):
        gdp.run_with_adaptive_time_step_plus_ivp_solver(dt)
        fa.run_one_step()
...
def test_wt_above_surface_adaptive_dt():
    """test that water tables above the topogrpahic elevation are
    set to the topographic elevation.

    Notes
    ----
    Same as ``test_wt_above_surface`` but uses the
...
def test_wt_above_surface_adaptive_dt_ivp():
    """test that water tables above the topogrpahic elevation are
    set to the topographic elevation.
    ...
    # initialize the groundwater model
    gdp = GroundwaterDupuitPercolator(grid, recharge_rate=0.0)
    gdp.run_with_adaptive_time_step_plus_ivp_solver(1)
    assert_equal(wt[4], 1)
...
def test_k_func():
    ...
    gdp1.run_with_adaptive_time_step_plus_ivp_solver(0)
    assert np.equal(0.005, gdp1.K).all()


def test_callback_func_adaptive_dt():
    """
...
def test_callback_func_adaptive_dt_ivp():
    """
    Test the use of a callback function to return the storage and
    substep durations while using the
    run_with_adaptive_time_step_plus_ivp_solver method.
    ...
    # run groundawter model
    gdp.run_with_adaptive_time_step_plus_ivp_solver(1e5)
