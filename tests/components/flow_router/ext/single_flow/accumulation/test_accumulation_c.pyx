...
def test_add_to_upstream_ordered_nodes():
    ...
    cdef:
        ...
        cnp.int64_t [:] upstream_ordered_nodes = np.zeros(16, dtype=int)
        cnp.int64_t [:] donors_start_indexes = np.array([0, 1, 2, 3, 4,
            4, 5, 6, 8, 9])
        cnp.int64_t [:] donors = np.array([0, 1, 2, 3, 5, 6, 4, 7, 8])
...
def test_calc_upstream_order_for_nodes_c():
     # on 25 nodes
    cdef:
        cnp.int64_t[:] base_level_and_closed_nodes = np.array([0, 1, 2, 4,
            5, 9, 10, 14, 15, 19, 20, 21, 22, 23, 24, 3])
        cnp.int64_t[:] upstream_ordered_nodes = -1 * np.ones(25, dtype=int)
        cnp.int64_t[:] donors_start_indexes = np.array([0, 1, 3, 5, 6,
            7, 8, 8, 8, 8, 10, 12, 12, 12, 13, 15, 16,
           17, 17, 17, 18, 19, 21, 22, 24, 25])
        cnp.int64_t[:] donors = np.array([0, 1, 6, 2, 7, 3, 4, 5, 8,
...
def test_calc_drainage_areas():
    # on 25 nodes
    cdef:
        cnp.int64_t [:] downstream_ordered_nodes = np.array([3, 24, 18, 23, 22,
            17, 16, 21, 20, 19, 15, 12, 13, 14, 11, 10, 8,
            9, 5, 4, 7, 2, 6, 1, 0])
        cnp.int64_t [:] receivers = np.array([0, 1, 2, 3, 4, 5, 1, 2, 9,
            9, 10, 10, 13, 14, 14, 15, 21,
           16, 23, 19, 20, 21, 22, 23, 24])
        cnp.float64_t [:] drainage_areas = np.array([0., 0., 0.,
            0., 0., 0., 900., 900., 900.,
