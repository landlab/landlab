...
def test_depth_and_grains_mass_match():
    ...
    mass_converted_to_dz_at_node = (np.sum(mg.at_node['grains__weight'][9]) / (
                mg.dx * mg.dy * soil_grading._soil_density * (1 - soil_grading._phi)))  # Take into account the porosity
...
def test_match_to_user_defined_median_size():
    ...
    soil_grading = SoilGrading(mg, meansizes=meansizes, initial_median_size=initial_median_size)

    median_size_at_node = mg.at_node['median_size__weight'][0]

    testing.assert_array_equal(median_size_at_node, initial_median_size,
                               err_msg=err_msg)
...
def test_match_to_user_defined_meansizes():
    ...
    testing.assert_array_equal(mg.at_node['grains_fractions__size'][9], meansizes,
                               err_msg=err_msg)


@pytest.mark.filterwarnings("ignore:Median size provided is larger than the distribution upper bound")
def test_beyond_range_user_defined_soil_median_size_larger():
    ...
    proportion_in_larger_grainsize_fraction = mg.at_node['grains__weight'][9,-1] / np.sum(mg.at_node['grains__weight'][9])

    testing.assert_approx_equal(proportion_in_larger_grainsize_fraction, 1,
                                err_msg=err_msg)
...
def test_beyond_range_user_defined_bedrock_median_size_larger():
    ...
    proportion_of_mass_in_the_larger_size_fraction = mg.at_node['bed_grains__proportions'][9,-1]

    testing.assert_approx_equal(proportion_of_mass_in_the_larger_size_fraction,1,
                                err_msg = err_msg)

@pytest.mark.filterwarnings("ignore:Median size provided is smaller than the distribution lower bound")
def test_beyond_range_user_defined_soil_median_size_smaller():
    ...
    proportion_in_smallest_grainsize_fraction = mg.at_node['grains__weight'][9,0] / np.sum(mg.at_node['grains__weight'][9])
    testing.assert_approx_equal(proportion_in_smallest_grainsize_fraction, 1,
                                err_msg=err_msg)
