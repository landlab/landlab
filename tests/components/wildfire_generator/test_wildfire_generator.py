...
def test_negative_fuel_raises():
    """fuel_availability containing negative values should raise ValueError."""
    mg = _make_grid()
...
# =============================================================================
# __init__ -- seed=None branch
# =============================================================================
 
 
def test_seed_none_does_not_crash():
    """Passing seed=None should initialise the component without error."""
    mg = _make_grid()
...
def test_rivers_mask_based_on_minimum_river_threshold():
    ...
    wg = WildfireGenerator(mg)
    assert wg._rivers[20] == True   # noqa: E712
    assert wg._rivers[21] == False  # noqa: E712
 
 
def test_fire_sizes_property():
    """fire_sizes should return a list of positive floats, one per fire."""
    mg = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.8)
    wg = WildfireGenerator(mg, potential_fires=50, aridity=0.7, seed=42)
    wg.run_one_step(1)
    assert isinstance(wg.fire_sizes, list)
    assert len(wg.fire_sizes) == len(wg.fire_log)
    assert all(s > 0 for s in wg.fire_sizes)
 
 
def test_burned_nodes_property():
    """burned_nodes should return a list of lists, one per fire."""
    mg = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.8)
    wg = WildfireGenerator(mg, potential_fires=50, aridity=0.7, seed=42)
    wg.run_one_step(1)
    assert isinstance(wg.burned_nodes, list)
...
def test_severity_property():
    """severity should return a list of floats in [0, 1], one per fire."""
    mg = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.8)
    wg = WildfireGenerator(mg, potential_fires=50, aridity=0.7, seed=42)
    wg.run_one_step(1)
    assert isinstance(wg.severity, list)
    assert len(wg.severity) == len(wg.fire_log)
    assert all(0.0 <= s <= 1.0 for s in wg.severity)
 
 
def test_ignition_nodes_property():
    """ignition_nodes should return a list of coordinates, one per fire."""
    mg = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.8)
    wg = WildfireGenerator(mg, potential_fires=50, aridity=0.7, seed=42)
    wg.run_one_step(1)
    assert isinstance(wg.ignition_nodes, list)
...
def test_last_step_fires_all_from_current_year():
    ...
    wg.run_one_step(1)
    wg.run_one_step(1)
    assert isinstance(wg.last_step_fires, list)
...
def test_get_regrowth_params_out_of_range_raises():
    """_get_regrowth_params should raise ValueError when _aridity is out of [0, 1]."""
    mg = _make_grid()
    wg = WildfireGenerator(mg)
    wg._aridity = 1.5  # bypass setter deliberately
    with pytest.raises(ValueError, match="out of range"):
        wg._get_regrowth_params()
...
def test_get_aridity_weights_out_of_range_raises():
    """_get_aridity_weights should raise ValueError when _aridity is out of [0, 1]."""
    mg = _make_grid()
    wg = WildfireGenerator(mg)
    wg._aridity = 1.5  # bypass setter deliberately
    with pytest.raises(ValueError, match="out of range"):
        wg._get_aridity_weights()
...
def test_calc_severity_at_zero_aridity_near_zero():
    """With aridity near 0, mean severity should be very close to 0."""
    mg = _make_grid(nrows=9, ncols=9, topo_val=100)
...
def test_calc_severity_at_unit_aridity_near_one():
    """With aridity = 1, mean severity should be close to 1."""
    mg = _make_grid(nrows=9, ncols=9, topo_val=100)
...
def test_regrow_vegetation_changing_dt():
    """Growth increment should match the analytic formula for each dt."""
...
def test_get_neighbors_returns_valid_nodes():
    ...
    mg = _make_grid(nrows=9, ncols=9, topo_val=100)
    wg = WildfireGenerator(mg)
    interior_node = 40  # centre of 9x9
...
def test_get_neighbors_interior_node_count():
    """A fully interior node on a 9x9 grid should have exactly 4 active D4 neighbors."""
    mg = _make_grid(nrows=9, ncols=9, topo_val=100)
    wg = WildfireGenerator(mg)
...
def test_fire_no_ignitions_produces_no_record():
    """When the Poisson draw returns 0, _fire should return immediately
    with no fire_log entries."""
    mg = _make_grid(nrows=9, ncols=9, fuel_val=0.8)
    wg = WildfireGenerator(mg, potential_fires=1, seed=0)
    wg._current_time = 1
    np.random.seed(0)
    wg._fire(dt=1e-10)  # poisson(1e-10) is almost certainly 0
...
def test_fire_ignition_skipped_on_river_center():
    ...
    mg = _make_grid(nrows=9, ncols=9, topo_val=100, fuel_val=1.0)
    mg.at_node["drainage_area"][mg.core_nodes] = 1e9  # all core nodes are rivers
...
def test_fire_low_ignition_prob_produces_fewer_fires():
    """With very low fuel and aridity, most ignition attempts should be rejected."""
    mg_low  = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.0)
    wg_low  = WildfireGenerator(mg_low,  potential_fires=50, aridity=0.01, seed=1)
    wg_low.run_one_step(1)
 
    mg_high = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=1.0)
    wg_high = WildfireGenerator(mg_high, potential_fires=50, aridity=0.9,  seed=1)
    wg_high.run_one_step(1)
 
    assert len(wg_high.fire_log) > len(wg_low.fire_log)
...
def test_fire_size_equals_core_cell_area_sum():
    """fire_size (km2) should equal the sum of cell areas of burned nodes / 1e6.
...
def test_fire_log_aridity_matches_component_aridity():
    ...
    mg = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.8)
    wg = WildfireGenerator(mg, potential_fires=50, aridity=0.65, seed=42)
    wg.run_one_step(1)
    for r in wg.fire_log:
...
def test_run_one_step_explicit_current_time_sets_correctly():
    """Consecutive explicit current_time calls should each set correctly."""
    mg = _make_grid()
    wg = WildfireGenerator(mg, seed=None)
    wg.run_one_step(1, current_time=50)
    assert wg._current_time == 50
    wg.run_one_step(1, current_time=99)
    assert wg._current_time == 99
...
def test_run_one_step_fire_log_year_matches_current_time():
    """Every fire_log entry's year should be within the simulated range."""
    mg = _make_grid(nrows=9, ncols=9, spacing=100, fuel_val=0.8)
