...
from landlab.components import RiverBedDynamics, OverlandFlow
...
def test_var_units(r_b_d):
    assert set(r_b_d.input_var_names) | set(r_b_d.output_var_names) | set(r_b_d.optional_var_names)     == set( 
    dict(r_b_d.units).keys() 
    )
...
def test_r_b_d_approximate_solution():
    ...
    of = OverlandFlow(grid, dt_max=max_dt,h_init=0.001,mannings_n=n,use_user_defined_time_step=True)
    ...
    grid['link']['surface_water__depth'] =  map_mean_of_link_nodes_to_link(grid,'surface_water__depth')
    grid['link']['surface_water__velocity'] = map_mean_of_link_nodes_to_link(grid,'surface_water__velocity')
    grid['link']['surface_water__velocity_previous_time'] = map_mean_of_link_nodes_to_link(grid,'surface_water__velocity_previous_time')
    rbd = RiverBedDynamics(grid , gsd = gsd, variable_critical_shear_stress = True, outlet_boundary_condition='fixedValue')

    # Set boundaries as closed boundaries, the outlet is set to an open boundary. 
    grid.set_watershed_boundary_condition_outlet_id([1,2], z, 45.)
    ...
    grid['node']['bed_surface__elevation_fixed'] = fixed_nodes # Assigns fixed locations to landlab grid 

    # Create bed and flow initial condition
    # Create bed and flow initial condition
    grid["link"]["surface_water__discharge"][link_inlet] = 50    # Flow discharge in m3/s/m
    ...
    grid['link']['sediment_transport__sediment_supply_imposed'][link_inlet] = upstream_sediment_supply
    ...
    while t < simulation_max_time:
        ...
        grid["link"]["surface_water__discharge"][link_inlet] = -1    # Flow discharge in m3/s/m
        grid["node"]["surface_water__depth"][node_inlet] = 0.45      # Flow depth in nodes in m
        grid["link"]["surface_water__depth"][link_inlet] = 0.45      # Flow depth in links in m
        ...
        for i in np.arange(0,calculated_nodes_Id.shape[0]):
            grid['node']['topographic__elevation'][calculated_nodes_Id[i,1:number_columns-1]] = z[calculated_nodes_Id[i,1:number_columns-1] - 2*number_columns] + 2 * grid.dx * bedSlope
            
        t = t + of.dt

    z = np.reshape(z,dem.shape)[:,1]
    x = np.arange(0,1700,50, dtype=np.int64)
    z_solution_approx = 2.066E-12*x**4 - 5.579E-09*x**3 + 4.714E-06*x**2 + 1.370E-02*x - 6.830E-01
