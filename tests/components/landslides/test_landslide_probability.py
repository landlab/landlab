...
(_SHAPE, _SPACING, _ORIGIN) = ((5, 4), (10e0, 10e0), (0.0, 0.0))
...
def test_calculate_landslide_probability_uniform_depth_method():
    ...
    ls_grid = landslide_pars_ongrid(grid,unit_default_value) #see default values given in dictionary above
    ...
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][5], 0.84
    )
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][9], 0.52
    )
...
def test_calculate_landslide_probability_uniform_recharge_method():
    ...
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][9], 0.92
    )
...
def test_calculate_landslide_probability_lognormal_depth_method():
    ...
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][9], 0.6
    )
...
def test_calculate_landslide_probability_lognormal_recharge_method():
    ...
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][9], 0.88
    )
...
def test_calculate_landslide_probability_lognormal_spatial_depth_method():
    ...
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][5], 0.72
    )
    np.testing.assert_almost_equal(
...
def test_calculate_landslide_probability_lognormal_spatial_recharge_method():
    ...
    np.testing.assert_almost_equal(
        ls_prob.grid["node"]["landslide__probability_of_failure"][9], 1.0
    )
...
def test_calculate_landslide_probability_datadriven_spatial_depth_method():
    ...
    ls_prob = LandslideProbability(ls_grid,number_of_iterations=25,
...
def test_calculate_landslide_probability_datadriven_spatial_recharge_method():
    ...
    for vkey in range(2,8):  #the first example vkey =2 is printed below
        HSD_dict[vkey] = np.random.uniform(Default_R[0],Default_R[1],10)    # 20 - 120 mm/day 
    HSD_id_dict = {}
