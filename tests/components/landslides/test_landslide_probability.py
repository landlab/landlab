...
def test_build_grid_unitarea():
    ...
    ls_prob_defaultpar = landslide_pars_ongrid(grid, gridnum, unit_default_value) #see default values given in dictionary above
...
def test_build_unitgrid(shape, spacing):
    ...
    gridnum=grid.number_of_nodes
    unit_default_value = calc_unit_default_value()
    ls_prob = landslide_pars_ongrid(grid, gridnum, unit_default_value) #see default values given in dictionary above
...
def test_field_initialized_to_range(ls_prob,unit_default_value):        
    ...
    for name in ls_prob.input_var_names:
        ...
        if name == 'soil__maximum_total_cohesion':
            assert field_min >= unit_min
            assert field_max <= unit_max +10 #set to max scatter applied to max cohesion
        else:
            assert field_min >= unit_min
            assert field_max <= unit_max
...
def test_testpars_unitlandslide(ls_prob):
    ...
    unit_default_value = calc_unit_default_value()
    ls_grid= landslide_pars_ongrid(ls_prob.grid, gridnum, unit_default_value) #see default values given in dictionary above
...
def test_calculate_landslide_probability_uniform_depth_method():
    ...
    grid, gridnum = unitgrid(dimensions)  
    unit_default_value = calc_unit_default_value()
    ls_grid = landslide_pars_ongrid(grid, gridnum, unit_default_value) #see default values given in dictionary above
