...
from landlab.components import FlowAccumulator, FlowDirectorMFD2
...
def test_MFD_SW_slope():
    ...
    fa = FlowAccumulator(mg, flow_director="FlowDirectorMFD2")
    fa.run_one_step()
...
def test_MFD_SW_slope_w_diags():
    mg = RasterModelGrid((10, 10))
    mg.add_field("topographic__elevation", mg.node_y + mg.node_x, at="node")
    fa = FlowAccumulator(mg, flow_director="FlowDirectorMFD2", diagonals=True)
    fa.run_one_step()
