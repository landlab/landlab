...
calc_water_mass = round(
    np.abs((np.trapz(hydrograph_time_sec, (np.abs(discharge_at_outlet) * rmg.dx)))), 2
)
theoretical_water_mass = round(
    (
        (rmg.number_of_core_nodes * rmg.cellarea)
        * (4.07222 * (10 ** -7))
        * storm_duration
    ),
    2,
...
percent_error = round(
    ((np.abs(calc_water_mass) - theoretical_water_mass) / theoretical_water_mass * 100),
    2,
)
