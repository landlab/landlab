...
for i in range(nt):
    ...
    theta_vE[core] = np.arctan(
        np.fabs(pgradEy[core]) / (np.fabs(pgradEx[core]) + 1.e-10)
    )
    ...
    theta_vW[core] = np.arctan(
        np.fabs(pgradWy[core]) / (np.fabs(pgradWx[core]) + 1.e-10)
    )
    ...
    theta_vN[core] = np.arctan(
        np.fabs(pgradNy[core]) / (np.fabs(pgradNx[core]) + 1.e-10)
    )
    ...
    theta_vS[core] = np.arctan(
        np.fabs(pgradSy[core]) / (np.fabs(pgradSx[core]) + 1.e-10)
    )
    ...
    hR[core] += dtwidth * (
        qsedS[core] + qsedW[core] - qsedN[core] - qsedE[core] + qsed_inR[core]
    )
