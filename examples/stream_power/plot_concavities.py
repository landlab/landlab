...
for i in xrange(len(limit_of_steepening)):
    ...
    if lim != 0:
        polyvals = np.polyfit(
            np.log10(A_profiles[i][start_node:lim]),
            np.log10(S_profiles[i][start_node:lim]),
            1,
        )
        if polyvals[0] < 0:
...
plot(
    ...
    np.log(aparabolicNMG2x10_theta[1:]),
    "x-",
    label="10*U,2*K,10*dU",
)
plot(
    aparabolicNMG2x5_propx, np.log(aparabolicNMG2x5_theta), "x-", label="10*U,2*K,5*dU"
)
plot(
    aparabolicNMG4x100_propx,
    np.log(aparabolicNMG4x100_theta),
    "x-",
    label="0.1*U,100*dU",
)
plot(
    aparabolicNMG4x10_propx, np.log(aparabolicNMG4x10_theta), "x-", label="0.1*U,10*dU"
)
plot(
    aparabolicNMG5x10_propx, np.log(aparabolicNMG5x10_theta), "x-", label="0.2*K,10*dU"
)
plot(
    bevel2seddep_propx[1:],
    np.log(bevel2seddep_theta[1:]),
    "x-",
    label="bevel_highangle",
)  # first val is a bad pick
plot(pureDL_nothresh_propx, np.log(pureDL_nothresh_theta), ".-", label="pureDL")
plot(
    stormsDL_thresh_propx,
    np.log(stormsDL_thresh_theta),
    "+-",
    label="pureDL_storms_threshold",
)
plot(
    stormsDL_nothresh_propx,
    np.log(stormsDL_nothresh_theta),
    "+-",
    label="pureDL_storms",
)
