...
class NearestNodeFinder:
    ...
    def __call__(self, coords_of_target) -> np.ndarray:
        ...
        array([0])
        """
        coords_of_target = np.asarray(coords_of_target, dtype=float)
        if (
            coords_of_target.ndim not in (1, 2)
            or coords_of_target.shape[-1] != self._tree.m
        ):
            raise ValueError(
                "`coords_of_target` must have shape (n_dims,) or "
                f"(n_targets, n_dims), got {coords_of_target.shape}."
            )

        return_scalar = coords_of_target.ndim == 1

        coords_of_target = np.atleast_2d(coords_of_target)
        _, nearest_nodes = self._tree.query(coords_of_target, k=1)

        return nearest_nodes[0] if return_scalar else nearest_nodes
