...
def neighbor_to_arrow(x, diagonals_ok=True):
    """Convert an array of neighbor nodes to flow direction arrows.

    Parameters
    ----------
    x : array_like
        Indices to neighbor elements.
    ...
    The default it to include diagonal neighbors.
    ...
    shift_row = x // n_cols - np.arange(n_rows).reshape((-1, 1))
    shift_col = x % n_cols - np.arange(n_cols)

    def get_arrow(row_shift, col_shift):
        return direction_arrows.get((row_shift, col_shift), "?")

    return np.vectorize(get_arrow)(shift_row, shift_col)
