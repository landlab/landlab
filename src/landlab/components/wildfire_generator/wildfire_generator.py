...
class WildfireGenerator(Component):
    ...
    **Fire ignition and spread**
    ...
    (D4) neighbours using the same fuel-aridity probability, modulated by a
    ...
    **Additional References**
         ...
         Pausas, J. G., & Paula, S. (2012). Fuel shapes the fire–climate relationship: evidence from Mediterranean ecosystems. Global Ecology and Biogeography, 21(11), 1074–1082. https://doi.org/10.1111/j.1466-8238.2012.00769.x

         Pausas JG, Ribeiro E. 2013. The global fire–productivity relationship. Global Ecology and Biogeography 22: 728-736. https://onlinelibrary.wiley.com/doi/abs/10.1111/geb.12043 
         ...
         Viedma, O., Meliá, J., Segarra, D., & Garcia-Haro, J. (1997). Modeling rates of ecosystem recovery after fires by using landsat TM data. Remote Sensing of Environment, 61(3), 383–398. https://doi.org/10.1016/S0034-4257(97)00048-5
    ...
    def __init__(self,
        ...
        self._grid.at_node["last_fire_time"][:] = -np.inf # Initialize last_fire_time to -inf (no fire has occurred yet)
        self._last_fire_time = self._grid.at_node["last_fire_time"]  

        self._regrowth_table = [
            (0.0, 0.1, 5.4, 0.42),  # Trees + shrubs, data derived from Viedma et al. 1997
        ...
        self._aridity_weights = [
            (0.0, 0.1, 0.5, 0.5), # Trees + shrubs, data derived from Viedma et al. 1997
    ...
    def _regrow_vegetation(self, dt):
        ...
        self._vegetation[:] = np.minimum(
            self._vegetation + delta,
            self._max_vegetation
        )
    ...
    def _fire(self, dt):
        ...
        for _ in range(fire_ignitions):
            ...
            ignition_prob = (self._vegetation[center] * fuel_weight +
                ...
                for node in fire_front:
                    ...
                    for neighbor in self._get_neighbors(node):
                        if (neighbor in burned
                                or neighbor == -1
                                or self._drainage_area[neighbor] > self._minimum_river_threshold):
                            continue
                        dx = self._grid.x_of_node[neighbor] - self._grid.x_of_node[node]
                        dy = self._grid.y_of_node[neighbor] - self._grid.y_of_node[node]
                        slope = (self._topo[neighbor] - self._topo[node]) / np.sqrt(dx**2 + dy**2)
                        slope_factor = 1 / (1 + np.exp(-self._alpha * slope))
                        spread_prob = (self._vegetation[neighbor] * fuel_weight +
                                    self._aridity * aridity_weight) * slope_factor
            ...
            veg_change = (1 - np.mean(self._vegetation[changed_nodes][non_zero_veg] / veg_before[non_zero_veg])) * 100
