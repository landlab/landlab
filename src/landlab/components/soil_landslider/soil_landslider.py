...
Propagating landslide failure erosional areas uphill of critical cells, entirely within the uppermost soil layer
and
Depositing landslide material downhill according to a multi-path flow routing algorithm
 
This component is built on the architecture of the BedrockLandslider component devoloped by Benjamin Campforts 
...
#			
    ...
    def _landslide_erosion(self, dt):
        ...
        if self._critical_sliding_nodes is None:
            ...
            # sliding (nodes where a slide may be triggered) is set by the temporal probability, but only occurs if the cell is unstable
            ...
            if slope_neighbors_to_crit_node.size > 0:
                ...
                while upstream_neighbors.size > 0 and (
                    ...
                    if new_el_1 < topo[upstream_neighbors[0]]:
                        ...
                        # add in the max step height constraint (this may be removed in future versions)
                        ...
                        # instead of setting the new elevation to the new el, set it to how much sediment was eroded
                        topo[upstream_neighbors[0]] = topo[upstream_neighbors[0]]-sed_landslide_ero
                        #print(f'cell {upstream_neighbors[0]} updated topo {topo[upstream_neighbors[0]]}')
                        
                        # there is no bedrock erosion
                        vol_sed = (sed_landslide_ero * (1 - self._phi) * (self.grid.dx**2))
                        # Troubleshooting
