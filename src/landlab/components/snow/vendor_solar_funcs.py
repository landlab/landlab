...
def Declination(day_angle, DEGREES=False, DMS=False):
    ...
    if DMS:
        ...
        delta = (
            deg + (min / np.float64(100)) + (sec / np.float64(10000))
        )  # [decimal DMS, DD.MMSS]
...
def Direct_Radiation_Flux(lat_deg, Julian_day, W_p, th, gamma_dust=None):
    ...
    tau = Atmospheric_Transmissivity(lat_deg, Julian_day, W_p, th, gamma_dust)
...
def Diffuse_Radiation_Flux(lat_deg, Julian_day, W_p, th, gamma_dust=None):
    ...
    gam_s = Scattering_Attenuation(
        lat_deg, Julian_day, W_p, th, gamma_dust
    )  # [unitless]
    K_ET = ET_Radiation_Flux(lat_deg, Julian_day, th)
...
def Global_Radiation_Flux(lat_deg, Julian_day, W_p, th, gamma_dust=None):
    ...
    K_dir = Direct_Radiation_Flux(lat_deg, Julian_day, W_p, th, gamma_dust)

    K_dif = Diffuse_Radiation_Flux(lat_deg, Julian_day, W_p, th, gamma_dust)
...
def BS_Radiation_Flux(lat_deg, Julian_day, W_p, albedo, th, gamma_dust=None):
    ...
    gam_s = Scattering_Attenuation(lat_deg, Julian_day, W_p, th, gamma_dust)

    K_global = Global_Radiation_Flux(lat_deg, Julian_day, W_p, th, gamma_dust)
...
def Clear_Sky_Radiation(
    lat_deg, Julian_day, W_p, TSN_offset, alpha, beta, albedo, gamma_dust
):
    # --------------------------------------------------------
    ...
    # **     gamma_dust = Dust_Attenuation()
    # --------------------------------------------------------
    tau = Atmospheric_Transmissivity(lat_deg, Julian_day, W_p, TSN_offset, gamma_dust)
    K_ET = ET_Radiation_Flux_Slope(lat_deg, Julian_day, TSN_offset, alpha, beta)
...
def Latitude_Grid(info):
    ...
    if info.pixel_geom == 0:
        ...
        lats = (
            (np.arange(info.nrows, dtype="float64") * dy) + info.y_south_edge + (dy / 2)
        )
...
def Longitude_Grid(info):
    ...
    if info.pixel_geom == 0:
        ...
        lons = (
            (np.arange(info.ncols, dtype="float64") * dx) + info.x_west_edge + (dx / 2)
        )
