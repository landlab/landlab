...
class VoronoiDelaunayToGraph(VoronoiDelaunay):
    def __init__(self, xy_of_node, perimeter_links=None):
        xy_of_node = require_array(
            np.asarray(xy_of_node, dtype=float),
            dtype=np.floating,
            shape=("n_nodes", 2),
            name="xy_of_node",
        )
        require_length_at_least(xy_of_node, 3, name="xy_of_node")

        if perimeter_links is not None:
            perimeter_links = require_array(
                np.asarray(perimeter_links),
                dtype=np.integer,
                shape=("n_links", 2),
                name="perimeter_links",
            )
            require_length_at_least(perimeter_links, 3, name="perimeter_links")
