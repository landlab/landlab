...
import matplotlib
from numpy import amin, exp, where
from pylab import clf, figure, show
...
class TurbulentSuspensionAndBleachingModel(OrientedRasterCTS):
    ...
    def __init__(self, input_stream):
        ...
        self.plot_interval = params.read_float(
            "plot_interval"
        )  # interval for plotting output, s
        self.run_duration = params.read_float(
            "model__run_time"
        )  # duration of run, sec (CSN)
        self.report_interval = params.read_float(
            "model__report_interval"
        )  # report interval, in real-time seconds
        ...
        super(TurbulentSuspensionAndBleachingModel, self).__init__(
            mg, ns_dict, xn_list, node_state_grid, prop_data=self.osl
        )
    ...
    def setup_transition_list(self):
        ...
        xn_list.append(
            Transition(
                (0, 1, 0), (1, 0, 0), 10., "left motion", True, self.update_bleaching
            )
        )
        xn_list.append(
            Transition(
                (1, 0, 0), (0, 1, 0), 10., "right motion", True, self.update_bleaching
            )
        )
        xn_list.append(
            Transition(
                (0, 1, 1), (1, 0, 1), 10.55, "down motion", True, self.update_bleaching
            )
        )
        xn_list.append(
            Transition(
                (1, 0, 1), (0, 1, 1), 9.45, "up motion", True, self.update_bleaching
            )
        )
