...
from landlab.grid.raster_steepest_descent import (
    calc_steepest_descent_across_adjacent_cells)
...
def test_scalar_arg():
    ...
    grad = calc_steepest_descent_across_adjacent_cells(
...
def test_scalar_arg_with_links():
    ...
    (grad, node) = calc_steepest_descent_across_adjacent_cells(
...
def test_node_id_in_direction_of_max():
    values = np.array([-1, 1,  3, 6, 10,
                       ...
                       0, 1,  3, 5, 10,
                       0, 1, -3, 6, 10, ], dtype=float)
    (_, node_ids) = calc_steepest_descent_across_adjacent_cells(
        rmg, values, (0, 4), return_node=True)
    assert_array_equal(node_ids, [5, 17])

    (grads,
     node_ids) = calc_steepest_descent_across_adjacent_cells(
...
def test_node_in_direction_of_max():
    for node_id in [0, 1, 2, 3, 5, 6, 7, 8]:
        values = np.zeros(9)
        values[node_id] = -1
        (_, node) = calc_steepest_descent_across_adjacent_cells(
