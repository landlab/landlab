...
def calc_grad_at_link(
    shape,
    xy_spacing,
    # cnp.ndarray[cnp.float64_t, ndim=1] value_at_node,
    # cnp.ndarray[cnp.float64_t, ndim=1] out,
    # double[:] value_at_node,
    # double[:] out,
    float_or_int[:] value_at_node,
    cython.floating[:] out,
):
    cdef int n_rows = shape[0]
    cdef int n_cols = shape[1]
    ...
    # _calc_grad_at_link(n_rows, n_cols, dx, dy, &value_at_node[0], &out[0])
...
cdef _calc_grad_at_link(
    ...
    for row in prange(0, n_rows - 1, nogil=True, num_threads=10, schedule="static"):
        # Horizontal links
        link = row * links_per_row
        node = row * n_cols
        for col in range(n_cols - 1):
