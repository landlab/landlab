...
class ClastCollection(ItemCollection):
    ...
    As perm ItemCollection, initializing ClastCollection creates a Pandas
...
###############################################################################
    ...
    def __init__(self,
        ...
        >>> CS.run_one_step(dt=50, Si=1.2, kappa=kappa, uplift=None,\
        ...
        4
        >>> np.greater(CS.DataFrame.at[0,'total_travelled_dist'], 0.0)
        True
...
#########################################
        ...
        # Add randomness for lateral spreading:
        if df.at[clast, 'target_node'] in _grid.boundary_nodes:
            [dx,dy] = [0,0] # to avoid clast going out of the grid
        else:
            ...
            df.at[clast, 'change_x'] = change_x #+ dx
            df.at[clast, 'change_y'] = change_y #+ dy
...
#####################################################
    ...
    def clast_solver_Exponential(self, dt=1., Si=1.2, kappa=0.0001,
                                 uplift=None, erosion_method='TLDiff'):
    ...
    def run_one_step(self, dt=1., Si=1.2, kappa=0.0001,
                     uplift=None, erosion_method='TLDiff'):
        ...
        erosion_method : str
            ...
            include:
                (1) 'TLDiff' (default): TransportLengthHillslopeDiffuser
                (2) 'Space': Space

        """
        self.clast_solver_Exponential(dt, Si, kappa, uplift, erosion_method)