...
def grid_ne():
    ...
    grid_ne.add_field('node',
                      'topographic__elevation',
                      (grid_ne.node_y+grid_ne.node_x)*-S);
    grid_ne.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_ne
...
def grid_nw():
    ...
    grid_nw.add_field('node',
                      'topographic__elevation',
                      (grid_nw.node_x-grid_nw.node_y)*S);
    grid_nw.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_nw
...
def grid_sw():
    ...
    grid_sw.add_field('node',
                      'topographic__elevation',
                      (grid_sw.node_x+grid_sw.node_y)*S);
    grid_sw.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_sw
...
def grid_se():
    ...
    grid_se.add_field('node',
                      'topographic__elevation',
                      (grid_se.node_y-grid_se.node_x)*S);
    grid_se.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_se
...
def grid_ene():
    ...
    grid_ene.add_field('node',
                      'topographic__elevation',
                      (grid_ene.node_y+2*grid_ene.node_x)*-S);
    grid_ene.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_ene
...
def grid_nne():
    ...
    grid_nne.add_field('node',
                      'topographic__elevation',
                      (2*grid_nne.node_y+grid_nne.node_x)*-S);
    grid_nne.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_nne
...
def grid_nnw():
    ...
    grid_nnw.add_field('node',
                      'topographic__elevation',
                      (grid_nnw.node_x-2*grid_nnw.node_y)*S);
    grid_nnw.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_nnw
...
def grid_wnw():
    ...
    grid_wnw.add_field('node',
                      'topographic__elevation',
                      (2*grid_wnw.node_x-grid_wnw.node_y)*S);
    grid_wnw.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_wnw
...
def grid_wsw():
    ...
    grid_wsw.add_field('node',
                      'topographic__elevation',
                      (2*grid_wsw.node_x+grid_wsw.node_y)*S);
    grid_wsw.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_wsw
...
def grid_ssw():
    ...
    grid_ssw.add_field('node',
                      'topographic__elevation',
                      (grid_ssw.node_x+2*grid_ssw.node_y)*S);
    grid_ssw.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_ssw
...
def grid_sse():
    ...
    grid_sse.add_field('node',
                      'topographic__elevation',
                      (2*grid_sse.node_y-grid_sse.node_x)*S);
    grid_sse.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_sse
...
def grid_ese():
    ...
    grid_ese.add_field('node',
                      'topographic__elevation',
                      (grid_ese.node_y-2*grid_ese.node_x)*S);
    grid_ese.set_closed_boundaries_at_grid_edges(bottom_is_closed=True,
                                                  left_is_closed=True,
                                                  right_is_closed=True,
                                                  top_is_closed=True)
    return grid_ese
...
#%matplotlib auto
#imshow_grid(grid_ne, 'topographic__elevation')
...
@pytest.fixture
def cc_ne(grid_ne):
    return ClastCollection(grid_ne,
...
@pytest.fixture
def cc_nw(grid_nw):
    return ClastCollection(grid_nw,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_sw(grid_sw):
    return ClastCollection(grid_sw,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_se(grid_se):
    return ClastCollection(grid_se,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_nne(grid_nne):
    return ClastCollection(grid_nne,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_nnw(grid_nnw):
    return ClastCollection(grid_nnw,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_wnw(grid_wnw):
    return ClastCollection(grid_wnw,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_wsw(grid_wsw):
    return ClastCollection(grid_wsw,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_ssw(grid_ssw):
    return ClastCollection(grid_ssw,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_sse(grid_sse):
    return ClastCollection(grid_sse,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
...
@pytest.fixture
def cc_ese(grid_ese):
    return ClastCollection(grid_ese,
                           clast_x=np.ones(3)*[1, 2, 3],
                           clast_y=np.ones(3)*2,
                           clast_elev=np.ones(3)*6,
                           clast_radius=np.ones(3)*0.5)
