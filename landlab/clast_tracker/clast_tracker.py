...
class ClastCollection(DataRecord):
    ...
    _cite_as = (
        """Insert a BibTeX formatted reference here"""
    )
    ...
    def __init__(
        ...
        self.x_of_corners_at_node = np.zeros(
            (self._grid.number_of_nodes, 4)
        )
        self.y_of_corners_at_node = np.zeros(
            (self._grid.number_of_nodes, 4)
        )
        ...
        _clast__cell = []
        _clast__cell[:] = self._grid.cell_at_node[
            _clast__node
        ]
        # format element_id:
        _element_id = np.array(
            (_clast__cell), dtype=int
        ).reshape((self._nb_of_clast, 1))
        ...
        for clast in range(self._nb_of_clast):
            if _clast__elev[clast] > (
                ...
                _clast__elev[clast] = self._grid.at_node[
                    "topographic__elevation"
                ][_clast__node[clast]]
        ...
        clast_data = {
            "clast__x": (
                ["item_id", "time"],
                clast_x.reshape((self._nb_of_clast, 1)),
            ),
            "clast__y": (
                ["item_id", "time"],
            ...
            "target_node": (
                ["item_id"],
                -np.ones(self._nb_of_clast, dtype=int),
            ),
            "target_node_flag": (
                ["item_id"],
                -np.ones(self._nb_of_clast, dtype=int),
    ...
    def _neighborhood(self, clast):
        ...
        self["lambda_0"][clast] = self.attrs["kappa"] / (
            2
            * self["clast__radius"][clast, -1]
            * (self.attrs["disturbance_fqcy"])
        )

        # If clast is phantom, slope is 0:
        if (
            ClastCollection.phantom(self, clast) is True
        ) or (self["clast__radius"][clast, -1] == 0):
            ...
            # Determine reference (closest) node for each clast:
            _grid = self._grid
            _node = self["clast__node"][
                clast
            ] = _grid.find_nearest_node(
                (
            ...
            _row_col_adjacent_nodes_at_node = _grid.neighbors_at_node[
                _node
            ]
            ...
            if any(
                i in _row_col_adjacent_nodes_at_node
                for i in _grid.boundary_nodes
            ) is True or (
                ClastCollection._cell_is_hillslope(
                    self, clast
                )
                is False
            ):
                ...
                we_slope = self["slope__WE"][
                    clast
                ].values.item()
                sn_slope = self["slope__SN"][
                    clast
                ].values.item()
                ...
                sn_norm = abs(sn_slope) / np.cos(
                    np.arctan(abs(sn_slope))
                )
                we_norm = abs(we_slope) / np.cos(
                    np.arctan(abs(we_slope))
                )
                ...
                if self.attrs["lateral_spreading"] == "on":
                    ...
                    slope_WE_with_dev = self["slope__WE"][
                        clast
                    ].values.item() * np.cos(
                        draw_dev[0]
                    ) - self[
                    ...
                    slope_SN_with_dev = self["slope__WE"][
                        clast
                    ].values.item() * np.sin(
                        draw_dev[0]
                    ) + self[
    ...
    def _move_to(self, clast):
        ...
        _node_z = _grid.at_node["topographic__elevation"][
            _node
        ]
        ...
        # flow__receiver_node provided by flow director:
            ...
            np.isnan(self["slope__WE"][clast]) is True
            and np.isnan(self["slope__SN"][clast]) is True
        ) or (self._cell_is_hillslope(clast) is False):
            target_node = self["target_node"][
                clast
            ] = _grid.at_node["flow__receiver_node"][_node]
            target_node_flag = self["target_node_flag"][
                clast
            ]
            ...
            if target_node_flag == -1:
                ...
                ss_horiz_norm = np.sqrt(
                    we_slope ** 2 + sn_slope ** 2
                )
                ss_dip = np.arctan(
                    (sn_slope ** 2 + we_slope ** 2)
                    / ss_horiz_norm
                )
                dx_to_exit = abs(
                    (_node_x + (self._grid.dx / 2))
                    - _clast_x
                )
                dy_to_exit = 0.0
                ...
                dist_to_exit = h_dist_to_exit / np.cos(
                    ss_dip
                )
                [change_x, change_y] = [
                    dx_to_exit + 1e-10,
            ...
            if we_slope == 0:
                if sn_slope == 0:
                    ...
                    if sn_slope < 0:
                        ...
                        target_node = _row_col_adjacent_nodes_at_node[
                            3
                        ]
                        [change_x, change_y] = [
                            0.0,
                ...
                if sn_slope == 0:
                    ...
                    if sn_slope > 0 and we_slope > 0:
                        ...
                        if (
                            ...
                            target_node = _diagonal_adjacent_nodes_at_node[
                                0
                            ]
                        ...
                        corner = 1
                        clast_to_corner_azimuth = np.radians(
                            180
                        ) - np.arctan(
                            np.abs(
                                _clast_y
        ...
        if (
            ...
            # Cell is hillslope:
            if (
                np.tan(self["slope__steepest_dip"][clast])
                >= self.attrs["Si"]
            ):
    ...
    def _move(self, clast):
        ...
        if np.isinf(self["distance__to_exit"][clast]):
            ...
            while (
                self["distance__to_travel"][clast].values
                > 0.0
            ):

                if (
                    ClastCollection.phantom(self, clast)
                    is False
                ):

                    if self["distance__to_travel"][
                        clast
                    ] >= (self["distance__to_exit"][clast]):
                        # clast leaves cell:
                        self["clast__x"][clast, -1] += self[
                            "change_x"
                        ][clast]
                        self["clast__y"][clast, -1] += self[
                            "change_y"
                        ][clast]
                        self["hop_length"][clast, -1] += (
                            np.sqrt(
                                np.power(
                                    self["change_x"].values[
                                        clast
                                    ],
                                    2,
                                )
                        ...
                        self["distance__to_travel"][
                            clast
                        ] -= self["distance__to_exit"][
                            clast
                        ]
                        # keep memory of lambda_mean:
                        _lambda_mean_old = self[
                            "lambda_mean"
                        ][clast]

                        # update new neighborhood and travel info:
                        ClastCollection._neighborhood(
                            self, clast
                        )
                        ClastCollection._move_to(
                            self, clast
                        )
    ...
    def _move_in_cell(self, clast):
        ...
        h_dist_to_trav = self["distance__to_travel"][
            clast
        ] * (np.cos(ss_dip))
        ...
        if ss_azimuth <= np.radians(90):
            ...
            [change_x, change_y] = [
                ...
                * np.cos(np.radians(180) - ss_azimuth),
                h_dist_to_trav
                * np.sin(np.radians(180) - ss_azimuth),
            ]
        elif ss_azimuth <= np.radians(270):
            [change_x, change_y] = [
                -h_dist_to_trav
                * np.sin(np.radians(270) - ss_azimuth),
                -h_dist_to_trav
            ...
            [change_x, change_y] = [
                ...
                * np.cos(np.radians(360) - ss_azimuth),
                -h_dist_to_trav
                * np.sin(np.radians(360) - ss_azimuth),
            ]
    ...
    def _clast_detach_proba(self, clast):
        """ Test if clast is detached (mobile)
        ...
        random_proba = np.random.uniform(
    ...
    def _cell_is_hillslope(self, clast):
        ...
        if (
            np.isnan(self["slope__steepest_azimuth"][clast])
            is True
        ):
            area_over_slope = 1
        else:
    ...
    def clast_solver_Exponential(
        ...
        if uplift is not None:
            if type(uplift) is str:
                self.attrs["uplift"] = self._grid.at_node[
                    uplift
                ]
            elif type(uplift) in (float, int):
                self.attrs["uplift"] = np.ones(
                ...
                raise TypeError(
                    "Supplied type of uplift is not recognized"
                )
        ...
        self.add_record(
            ...
            new_record={
                "clast__x": (
                    ["item_id", "time"],
                    np.full(1, np.NaN).reshape(1, 1),
                )
            },
        )
        # forward fill some of the values:
        self["clast__radius"] = self["clast__radius"].ffill(
            "time"
        )
        ...
        for clast in range(self._nb_of_clast):
            ...
            self["lambda_0"][clast] = self.attrs[
                "kappa"
            ] / (
                2
                * self["clast__radius"][clast, -1]
            ...
            if (
                ...
                if (
                    ClastCollection._clast_detach_proba(
                        self, clast
                    )
                    is True
                ):
                    ...
                    # Update elevation: set at node's elev
                    self["clast__elev"][
                        clast, -1
                    ] = self._grid.at_node[
                        "topographic__elevation"
                    ...
                    # To do: Clast is buried in the active layer with inv
                    # exp proba: elev - np.random.exponential(
                    # scale=self.attrs['d_star'], size=1)[0] or (
                    # self.attrs['d_star']/100 * (np.random.rand(1)))

                    # Update total travelled distance:
                    self["total_travelled_dist"][
                        clast
                    ] += self["hop_length"][clast, -1]
