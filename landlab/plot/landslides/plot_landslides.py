...
def test_field_initialized_to_range(ls_prob, input_var_names, unit_default_value):
    ...
    for name in input_var_names:
        ...
        print(
            "Input: (Min, Max) range of {value} model inputs: {pars}".format(
...
def print_nodevalues(ls_prob):
    ...
    for name in ls_prob.grid["node"]:
        ...
        print_minmax = print(
            "{value} (Min, Max) range of ({value1},{value2}) values on node:".format(
                value=name, value1=field_min, value2=field_max
            )
        )
        print_value = print(
            "All {value} default value initialized for each node:".format(value=name)
        )
        field = ls_prob.grid["node"][name]
        print(field)
