...
class find_facets(object):
    ...
    def define_aspect_node_subset_local(self, dist_tolerance=4., angle_tolerance=15., dip_dir='E'):
        ...
        diff_angles = np.min([np.fabs(angle_to_ft-self.aspect[subset]), np.fabs(np.fabs(angle_to_ft-self.aspect[subset])-2.*np.pi)], axis=0)
        ...
        #show()
        figure(6)
        gridshow.imshow_core_node_grid(self.grid, np.where(self.diff_angles<100000., self.diff_angles, -1.))
    ...
    def find_slope_lines(self, tolerance=1.):
        ...
        for i in unique_starting_pts:
            ...
            close_ft_nodes = np.less(ft_pt_distances_to_node,5.*grid.node_spacing)
            ...
            if nodes_possible.size>10.:
                ...
                pts_betw_large_diffs = np.diff(locs_of_large_diffs) #...this can be empty quite easily
