...
def test_stream_length_regular_grid_d8():
    ...
    flow_length_expected = np.reshape (flow_length_expected,mg.number_of_node_rows*mg.number_of_node_columns)
    ...
    flow_length = np.reshape(flow_length,mg.number_of_node_rows*mg.number_of_node_columns)
    # modifying the flow_length_map because boundary and outlet nodes should not
...
def test_stream_length_irregular_grid_d4():
    ...
    hmg.add_field('topographic__elevation', hmg.node_x + np.round(hmg.node_y), at = 'node')
    # instantiate the expected flow_length array
    flow_length_expected = np.array ([0,0,0,0,0,dx,0,0,dx,dx,2*dx,0,0,2*dx,2*dx,0,0,0,0])
    #setting boundary conditions   
...
def test_stream_length_raster_MFD_diagonals_true():
    ...
    mg.set_closed_boundaries_at_grid_edges(bottom_is_closed=True, left_is_closed=True, right_is_closed=True, top_is_closed=True)
    # calculating flow directions with FlowAccumulator component
    fa = FlowAccumulator(mg, 'topographic__elevation',flow_director='MFD',diagonals = True)
    fa.run_one_step()
    # calculating flow length map
    flow_length = calculate_stream_length(mg, add_to_grid=True, noclobber=False)
    # test that the stream length utility works as expected
...
def test_stream_length_raster_MFD_diagonals_false():
    ...
    mg.set_closed_boundaries_at_grid_edges(bottom_is_closed=True, left_is_closed=True, right_is_closed=True, top_is_closed=True)
    # calculating flow directions with FlowAccumulator component
    fa = FlowAccumulator(mg, 'topographic__elevation',flow_director='MFD',diagonals = False)
    fa.run_one_step()
    # calculating flow length map
    flow_length = calculate_stream_length(mg, add_to_grid=True, noclobber=False)
    # test that the stream length utility works as expected
