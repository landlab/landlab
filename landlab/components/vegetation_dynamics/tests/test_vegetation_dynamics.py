...
def test_input_var_names():
    assert sorted(Veg.input_var_names) == ['surface__evapotranspiration',
...
def test_output_var_names():
    assert sorted(Veg.output_var_names) == ['vegetation__cover_fraction',
...
@with_setup(setup_grid)
def test_var_units():
    assert set(Veg.input_var_names) | set(Veg.output_var_names) == set(dict(Veg.units).keys())
...
def test_field_getters():
    ...
    for name in Veg.grid['cell']:
        ...
        assert isinstance(field, np.ndarray)
        assert field.shape == (Veg.grid.number_of_cell_rows * Veg.grid.number_of_cell_columns, )
