        ...
        if node_Status[node_id]==0:
            ...
            if (q_loc > 0) :
                ...
                if (H_loc > thickness_lim or slope_loc <= 0 or   sed_erosion_loc==0):
                    # print('special case H: ' + str(H_loc))
...
def calculate_qs_in_lakeFiller(np.ndarray[DTYPE_INT_t, ndim=1] stack_flip_ud,
                    np.ndarray[DTYPE_INT_t, ndim=1] flow_receivers,
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] cell_area,
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] q,
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] qs,
                    ...
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] H,
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] br,
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] waterEl,
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] sed_erosion_term,                    
                    np.ndarray[DTYPE_FLOAT_t, ndim=1] bed_erosion_term,
                    DTYPE_FLOAT_t phi,
                    DTYPE_FLOAT_t F_f,                    
    ...
    for i in range(n_nodes):
        ...
        if (q_loc > 0) :
            qs_out = (qs_in[node_id] + Es[node_id]*cell_area[node_id] + (1.0-F_f)*Er[node_id]* cell_area[node_id]) / \
                            (1.0+(v_s*cell_area[node_id]/q[node_id]))
            depo_rate = v_s*qs_out/q[node_id]          
            H_loc       =   H[node_id]
            H_Before    =   H[node_id]
            slope_loc   =   slope[node_id] 
            sed_erosion_loc = sed_erosion_term[node_id]
            ...
            # if flooded and sediment should be dropped up to the water level
            if (fillLakesToBrim==1) and (slope_loc <0) and waterEl[node_id]!=-1:
                tot_qs = (qs_in[node_id] + Es[node_id]*cell_area[node_id] + (1.0-F_f)*Er[node_id]* cell_area[node_id]) 
                max_D = tot_qs*dt/ (1 - phi)
                H_loc = min(waterEl[node_id]-br[node_id],max_D)
            else: 
                # Correct for thick soils where soil thickness can grow to inf
                if (H_loc > thickness_lim or slope_loc <= 0 or   sed_erosion_loc==0):
                    # print('special case H: ' + str(H_loc))
