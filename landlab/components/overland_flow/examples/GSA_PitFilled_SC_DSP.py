...
sampling_sites = [43644, 50755, 54856, 58180, 59700]
...
dle = DepthSlopeProductErosion(rmg, k_e = (4.0 * (10**-9)), tau_crit=10.0)#.1)#232#704)
...
while elapsed_time < model_run_time:
   ...
   # outlet_tau_list.append(dle.tau[sampling_sites])
    
#==============================================================================
#     if y%20 == 0:
#         WSS = rmg['node']['water_surface__slope']
#         WSS[np.where(z == -9999)] = None
#         plt.figure('Water surface slope plots')
#         plt.subplot2grid((3,2), (0,0), colspan=2, rowspan=2)
#         DJET = cmap_discretize(cm.ocean_r, N=25)
#         fig = plt.imshow(WSS.reshape(rmg.shape), origin='left', cmap=DJET, vmin=0, vmax=0.25)
#         fig.axes.get_xaxis().set_visible(False)
#         fig.axes.get_yaxis().set_visible(False)
#         cb = plt.colorbar()
#     
#         #cb.ax.tick_params(**hfont)
#         cb.set_label('Discharge (cms)', rotation=270, labelpad=15)#, **hfont)
#         plt.subplot2grid((3,2), (2, 0), colspan=2)
#         discharge = np.array(np.abs(discharge_at_outlet)) * rmg.dx
#         hr = [x / 3600. for x in hydrograph_time_sec]
#         plt.plot(hr, discharge, 'k.')
#         plt.ylim(0, 40)
#         plt.ylabel('Discharge (cms)')#, **hfont)
#         plt.xlabel('Time (hrs)')#, **hfont)
#         plt.xlim(0, 8)
#         plt.show()
#     
#         plt.savefig(filenameS_template.format(y), format='png', dpi=100)
#         plt.close()
#         
# #        plt.figure('Tau lots')
# #        tau = dle.tau
# #        tau[np.where(z == -9999)] = None
# #        plt.figure(1)
# #        plt.subplot2grid((3,2), (0,0), colspan=2, rowspan=2)
# #        DJET = cmap_discretize(cm.viridis, N=25)
# #        fig = plt.imshow(tau.reshape(rmg.shape), origin='left', cmap=DJET, vmin=0, vmax=150)
# #        fig.axes.get_xaxis().set_visible(False)
# #        fig.axes.get_yaxis().set_visible(False)
# #        cb = plt.colorbar()
# #    
...
# #        plt.show()
# #    
# #        plt.savefig(filenameT_template.format(y), format='png', dpi=100)
#         plt.close()
#     
#     
#==============================================================================
