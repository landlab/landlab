...
Bates et al. (2012) algorithm for storage-cell inundation modeling.
...
h_links = mg.add_zeros('link', 'water_depth') + h_init # water depth at links
...
while elapsed_time <= run_time:
    ...
    # And now we add it to the second column, in all rows that are not
    # boundary rows.
    ...
    q[active_links] = ((q[active_links] - g * hflow * dtmax *
        water_surface_slope) / (1. + g * hflow * dtmax * n * n * abs(
        q[active_links]) / (hflow ** ten_thirds)))
...
# one row for plotting.  We will also remove the first (boundary) cell from
# this array, while also appending a [0] value at the end to keep it the same
# size as the 'x' array.
