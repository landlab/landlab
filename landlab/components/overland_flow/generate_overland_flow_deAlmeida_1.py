#Embedded file name: /Users/Jordan/Documents/landlab/landlab/components/overland_flow/generate_overland_flow_deAlmeida.py
...
class OverlandFlow(Component):
    """  Landlab component that simulates overland flow using the de Almeida et al., 2012 approximations
    ...
    landlab.components.overland_flow folder.
        ...
        input_file : Contains necessary and optional inputs. If not given, default input file and values is used.
    ...
    def __init__(self, grid, input_file = None, use_fixed_links=False, **kwds):
        ...
        self.h_links = grid.add_zeros('link', 'water_depth', units=self._var_units['water_depth'])
        ...
        # When we instantiate the class we recognize that neighbors have not been
        # found. After the user either calls self.set_up_neighbor_array or self.overland_flow
        # this will be set to True. This is done so that every iteration of self.overland_flow
    ...
    def set_up_neighbor_arrays(self, grid):
        ...
        self.north_neighbors = links.vertical_north_link_neighbor(grid.shape, self.vertical_active_link_ids)
        self.south_neighbors = links.vertical_south_link_neighbor(grid.shape, self.vertical_active_link_ids)
        
        # Using the horizontal active link ids, we can find the west and east neighbors
        self.west_neighbors = links.vertical_west_link_neighbor(grid.shape, self.horizontal_active_link_ids)
        self.east_neighbors = links.horizontal_east_link_neighbor(grid.shape, self.horizontal_active_link_ids)
    ...
    def overland_flow(self, grid, dt = None, **kwds):
        ...
        # water depths, topographic elevations and water discharge variables to the fields. 
        ...
        # Now we can calculate discharge. To handle links with neighbors that do not exist, we will do 
        # a fancy indexing trick. Non-existent links or inactive links have an index of '-1', which in
        # Python, looks to the end of a list or array. To accommodate these '-1' indices, we will simply
        # insert an value of 0.0 discharge (in units of L^2/T) to the end of the discharge array.
        ...
        # Now to return the array to its original length (length of number of all links), we delete
        # the extra 0.0 value from the end of the array.
