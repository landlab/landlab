...
class SoilMoisture( Component ):
    ...
    def __init__( self, grid, **kwds ):
        ...
        self._vegtype = \
          kwds.pop('VEGTYPE', np.zeros(grid.number_of_cells,dtype = int))       
    ...
    def update( self, current_time, **kwds ):
        ...
        for cell in range(0,self.grid.number_of_cells):
            ...
            Inf_cap = self._soil_Ib[cell]*(1-self._vegcover[cell]) + self._soil_Iv[cell]*self._vegcover[cell] # Infiltration capacity
            Int_cap = min(self._vegcover[cell]*self._interception_cap[cell], P)  # Interception capacity
