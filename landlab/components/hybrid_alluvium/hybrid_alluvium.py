...
class HybridAlluvium(Component):
    ...
    def run_one_step(self, dt=1.0, flooded_nodes=None, method=None, **kwds):
        ...
        self.soil__depth[(self.q > 0) & (blowup==True) & (self.slope > 0) & (flooded==True)] = (deposition_pertime[(self.q > 0) & (blowup==True) & (flooded==True)] / (1 - self.phi)) * dt   
        #non-positive slopes, not flooded
        self.soil__depth[(self.q > 0) & (blowup==True) & (self.slope <= 0) & (flooded==False)] += (deposition_pertime[(self.q > 0) & (blowup==True) & (self.slope <= 0) & (flooded==False)] / (1 - self.phi)) * dt    
        
        ##more general case:
        #positive slopes, not flooded
        self.soil__depth[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)] = self.H_star * np.log((1 / ((deposition_pertime[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)] * (1 - self.phi)) / (self.sed_erosion_term[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)]) - 1)) * (np.exp((deposition_pertime[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)] * (1 - self.phi) - (self.sed_erosion_term[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)]))*(dt / self.H_star)) * (((deposition_pertime[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)] * (1 - self.phi) / (self.sed_erosion_term[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)])) - 1) * np.exp(self.soil__depth[(self.q > 0) & (blowup==False) & (self.slope > 0) & (flooded==False)] / self.H_star)  + 1) - 1))
        #places where slope <= 0 but not flooded:
        self.soil__depth[(self.q > 0) & (blowup==False) & (self.slope <= 0) & (flooded==False)] += (deposition_pertime[(self.q > 0) & (blowup==False) & (self.slope <= 0) & (flooded==False)] / (1 - self.phi)) * dt     
        #flooded nodes:        
        self.soil__depth[(self.q > 0) & (blowup==False) & (flooded==True)] += (deposition_pertime[(self.q > 0) & (blowup==False) & (flooded==True)] / (1 - self.phi)) * dt     
