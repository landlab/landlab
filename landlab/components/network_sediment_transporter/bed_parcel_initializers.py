...
def BedParcelInitializerDischarge(BedParcelInitializerBase):
    """
    ...
    >>> from landlab.utils.parcels import BedParcelInitializerDischarge
...
def BedParcelInitializerDepth(BedParcelInitializerBase):
    """
    This function creates a landlab DataRecord to represent parcels of sediment
    on a river network (represented by a NetworkModelGrid). The function takes
...
def BedParcelInitializerArea(BedParcelInitializerBase):
    """
    This function creates a landlab DataRecord to represent parcels of sediment
    ...
    def __init__(
        ...
        **kwds
        ...
        if drainage_area_coefficient == None:
            raise ValueError("User must provide drainage_area_coefficient")

        if drainage_area_exponent == None:
...
def BedParcelInitializerUserD50(BedParcelInitializerBase):
    """
    This function creates a landlab DataRecord to represent parcels of sediment
    on a river network (represented by a NetworkModelGrid). The function
    takes a coefficient and exponent in a grain size-drainage area power law
    ...
    def calc_d50(self):
            ...
            if np.size(self._user_d50) == 1: # one d50, all links
                ...
                msg = ("user defined D50 must be either a scalar or size(element_id)"
                )
                raise ValueError(msg)
