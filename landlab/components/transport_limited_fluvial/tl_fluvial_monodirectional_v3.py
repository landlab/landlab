...
class TransportLimitedEroder(Component):
        ...
        ***Parameters for input file***
        ...
        if mannings_n < 0. or mannings_n > 0.2:
            print("***STOP. LOOK. THINK. You appear to have set Manning's n " +
                  "outside its typical range. It is set to "+str(mannings_n) +
        ...
        try:
            # ^fails if no Dchar provided, or shields crit is being set
        ...
        self.bad_neighbor_mask = np.equal(
            grid.get_neighbor_list(bad_index=-1), -1)
    ...
    def erode(self, grid, dt=None, node_elevs='topographic__elevation',
        ...
        *slopes_from_elevs* allows the module to create gradients internally
        ...
        'topographic__elevation']; set to 'name_of_field' to override this
        name, or pass an nnode-array to use those values as elevations instead.
        Using this option is considerably slower than any of the alternatives,
        as it also has to calculate the link_node_mapping from stratch each
        time.
        ...
        *Dchar_if_used* must be set as a grid field string or nnoodes-long
        array if 'Dchar' as a float was not provided in the input file.
        (If it was, this will be overridden).
        ...
        ***Note the time units are SECONDS in these fields***
        ...
        while 1:
            ...
            if stability_condition == 'tight':
                mock_diffusivities = np.zeros_like(transport_capacities,
                                                   dtype=float)
                ...
                if delta_t_internal == np.inf:  # no node exceeds crit
                    ...
                    # ^nothing happened, so let the loop complete, awaiting
                    # more uplift
            ...
            if stability_condition == 'loose':
                ...
                node_flattening = (self.fraction_gradient_change*elev_diff -
                                   delta_dz)
                # ^note the condition is that gradient may not change by >X%,
                # not must be >0
                # note all these things are zero for a pit node
                most_flattened_nodes = np.argmin(node_flattening[
                    grid.core_nodes])
                most_flattened_nodes = np.take(grid.core_nodes,
                                               most_flattened_nodes)
                # ^get it back to node number, not core_node number
                most_flattened_val = np.take(node_flattening,
                                             most_flattened_nodes)
                if most_flattened_val >= 0.:
                    break_flag = True  # all nodes are stable
                else:  # a fraction < 1
                    dt_fraction = self.fraction_gradient_change * np.take(
                        elev_diff, most_flattened_nodes)/np.take(
                            delta_dz, most_flattened_nodes)
                    # correct those elevs
                    dz *= dt_fraction
        ...
        if self.return_ch_props:
            ...
            H = (Q**(0.6*(1.-self._b))*(self.mannings_n/self.k_w)**0.6 *
                 node_S**-0.3)
