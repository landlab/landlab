...
class Clonal(Repro):
    ...
    def disperse(self, plants):
        ...
        runner_length[available_carb > 0] = (
            available_carb[available_carb > 0] / self.unit_cost
        )
        pup_dist = rng.uniform(
        ...
        plants["pup_x_loc"][filter] = (
            pup_dist[filter] * np.cos(pup_azimuth[filter]) + plants["x_loc"][filter]
        )
        plants["pup_y_loc"][filter] = (
            pup_dist[filter] * np.sin(pup_azimuth)[filter] + plants["y_loc"][filter]
        )
