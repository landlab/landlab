...
# We shouldn't have to index anything by j day I don't think. Since we are doing the loop outside of this
# j is current_day
            if self.current_day < self.gs_end:
            # don't have maintenance respiration coefficients defined 
            # Need to figure out how to read in light
            ...
            # Needs to be an else statement?
            if self.current_day >= self.gs_end:

                respMaint = 0
            
            #glucose requirement for growth
            # Need to define fraction of carbon that goes into plant part
            ...
            if self.current_day < self.gs_end # aren't we already in this time frame from the above if statement?
                #dailyphoto = []
