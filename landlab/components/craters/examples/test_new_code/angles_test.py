...
for i in xrange(loops):
    for j in xrange(nt):
        ...
        #if np.any(np.isnan(mg['node']['planet_surface__elevation'])):
        #    print np.where(np.isnan(mg['node']['planet_surface__elevation']))
    ...
    np.save(('r_'+str((i+1)*nt+offset)),r)
    np.save(('slope_'+str((i+1)*nt+offset)),slope)
    np.save(('angle_'+str((i+1)*nt+offset)),angle)
    np.save(('az_'+str((i+1)*nt+offset)),az)
    np.save(('mass_balance_'+str((i+1)*nt+offset)),mass_balance)

#instantiate the component:
craters_component = impactor(mg, './angles_params.txt')
