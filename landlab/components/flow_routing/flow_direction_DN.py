    ...
    if method == 'cython':
        ...
        if grid==None or not RasterModelGrid in inspect.getmro(grid.__class__):
            ...
            try:
                ...
                links_list[:, 4:6] = grid.diagonal_links_at_node[non_boundary_nodes, 2:0:-1]
                links_list[:, 6] = grid.diagonal_links_at_node[non_boundary_nodes, 0]
                links_list[:, 7] = grid.diagonal_links_at_node[non_boundary_nodes, 3]  # final order SW,NW,NE,SE
