...
class PrecipitationDistribution(Component):
    ...
    Inputs
    ------
    input_file : Contains necessary inputs. If not given, default input file
        is used.
        - MEAN_STORM: (type: float) the mean storm duration if not provided in
            initialization
        - MEAN_DEPTH: (type: float) the mean storm depth if not provided in
            initizalization
        - MEAN_INTERSTORM: (type: float) the mean interstorm duration if not
            provided in initialization
        - RUN_TIME: (type: float) total model run time if not provided in
            initialization
        - DELTA_T: (type: int) external time step increment if not provided in
            initialization. (It is not obligtory to provide a DELTA_T)

    So, without an input file (selecting the default), we can call this
    component like...
    ...
    model run time and delta t...
    Say we use 1.5 for mean storm, 15 for mean interstorm, 0.5 for mean depth,
    100 for model run time and 1 for delta t...
    ...
    def __init__(self, input_file=None, mean_storm=None, mean_interstorm=None,
        ...
        assigned, and creates an instantaneous storm event drawn from the
    ...
    def get_storm_time_series(self):
        ...
        respectively.
        :returns: array containing several sub-arrays of events [start, finish,
            intensity]


        There is a helper and an iterator. Open to suggestions on how to make
        this sleeker. helper keeps track of storm events so that we can
        properly adjust the time for the storm_time_series list.

        Storm iterator makes sure that the run goes through to the specified
        time, either read in as "run_time" by the ModelParameterDictionary or
        specified the fire time this method is called.
        """
        ...
        while storm_iterator <= self.run_time:
            ...
            next_storm_end = (next_storm_start +
                              round(self.get_precipitation_event_duration(),
                                    2))
            intensity = round(self.get_storm_intensity(), 2)
            self.get_storm_depth()
            self.storm_time_series.append([next_storm_start, next_storm_end,
                                           intensity])
    ...
    def yield_storm_interstorm_duration_intensity(self,
        ...
        while self._elapsed_time < self.run_time:
            ...
            self.get_storm_depth()
            intensity = self.get_storm_intensity()
            # ^this is a VELOCITY, i.e., a rainfall rate
            if (self._elapsed_time+storm_duration) > self.run_time:
                storm_duration = self.run_time-self._elapsed_time
            while (delta_t is not None) and (
                    (storm_duration-step_time) > delta_t):
