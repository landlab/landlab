class MWR_Saver():
    ...
    def prep_mw_data_containers(self, mw_i, mw_id):
        ...
        if self.MWR.track_attributes:
            self.att_r[mw_id] = dict.fromkeys(self.MWR._tracked_attributes, []) # this becomes the data container for each attribute
    ...
    def save_conditions_before_runout(self, mw_i, mw_id):
        ...
        if self.MWR.track_attributes:
            for key in self.MWR._tracked_attributes:
                self.att_r[mw_id][key].append(self.MWR._grid.at_node[key].copy()) # for each attribute, a copy of entire grid
                self.aratt_r[mw_id][key].append(self.MWR.aratt) 
        self.st_r[mw_id].append(self.MWR._grid.at_node['soil__thickness'].copy())
        self.tss_r[mw_id].append(self.MWR._grid.at_node['topographic__steepest_slope'].copy())
        self.frn_r[mw_id].append(self.MWR._grid.at_node['flow__receiver_node'].copy())
        self.frp_r[mw_id].append(self.MWR._grid.at_node['flow__receiver_proportions'].copy())
    ...
    def save_conditions_after_one_iteration(self, mw_i, mw_id):
        ...
        self.flowing_volume[mw_id].append(DEMdf_r.sum()*self.MWR._grid.dx*self.MWR._grid.dy)     
        self.runout_evo_maps[mw_i][self.MWR.c+1] = self.MWR._grid.at_node['energy__elevation'].copy()
        self.runout_evo_maps[mw_i][self.MWR.c+1] = self.MWR._grid.at_node['energy__elevation'].copy()
        self.topo_evo_maps[mw_i][self.MWR.c+1] = self.MWR._grid.at_node['topographic__elevation'].copy()
        if self.MWR.track_attributes:
            for key in self.MWR._tracked_attributes:
