...
from .cfuncs import _FD_D8, _FA_D8
    ...
    **Additional References**
    ...
    def __init__(
        ...
        if (flow_metric in PSINGLE_FMs) or (flow_metric in PMULTIPLE_FMs):
            ...
            raise ValueError(["flow metric should be one of these single flow directors : " + str(PSINGLE_FMs) + " or multiple flow directors" + str(PMULTIPLE_FMs)])
        ...
        if flow_metric in PMULTIPLE_FMs:      
            self.grid.at_node["topographic__steepest_slope"] = np.zeros((self.grid.number_of_nodes,8))
            self.grid.at_node["flow__receiver_node"] = np.zeros((self.grid.number_of_nodes,8),dtype=int)
            self.grid.at_node["flow__receiver_proportions"] = np.zeros((self.grid.number_of_nodes,8))
            self.grid.at_node["flow__link_to_receiver_node"] = np.zeros((self.grid.number_of_nodes,8))
        ...
        if self._seperate_Hill_Flow:
            if self._accumulateFlowHill:
                ...
                self._hill_discharges       = self.grid.at_node["hill_surface_water__discharge"]
            if hill_flow_metric in PMULTIPLE_FMs:      
                self.grid.at_node["hill_topographic__steepest_slope"] = np.zeros((self.grid.number_of_nodes,8))
                self.grid.at_node["hill_flow__receiver_node"] = np.zeros((self.grid.number_of_nodes,8),dtype=int)
                self.grid.at_node["hill_flow__receiver_proportions"] = np.zeros((self.grid.number_of_nodes,8))
    ...
    def _create_properties(self):
        self._deprFreeDEM = cp.deepcopy(rd.rdarray(
                    self.grid.at_node["topographic__elevation"].reshape(self.grid.shape),no_data=-9999) )
        

        # Calculate SQUARED length adjacent
        self.grid.at_node["SQUARED_length_adjacent"]=np.concatenate((np.ones((self.grid.number_of_nodes,4)),
                                                                        2*np.ones((self.grid.number_of_nodes,4))),axis=1)
    ...
    def calculateFlowDir_Acc(self, hillFlow=False, updateDepressions=True):
        ...
        if flowMetric == 'D8':
            ...
            props_Pf = rd.FlowProportions(dem=self._deprFreeDEM,
                                                       method=flowMetric,exponent=self._exponent)
            ...
            props_Pf = (props_Pf.reshape(props_Pf.shape[0]*props_Pf.shape[1],props_Pf.shape[2])) 
            props_Pf_col0 = props_Pf[:,0]
            props_Pf=np.column_stack((props_Pf[:,5], props_Pf[:,7],props_Pf[:,1], props_Pf[:,3],
                    props_Pf[:,6], props_Pf[:,8], props_Pf[:,2], props_Pf[:,4]))        
            rcvrs=np.concatenate((self._grid.adjacent_nodes_at_node,
    ...
    def FA_DA_D8(self, hillFlow=False):
        ...
        _FD_D8(receivers,distance_receiver,steepest_slope,np.array(el),el_ori ,dist,
                                          ngb,cores,activeCells, el_d,
                                          r,c,dx,sq2,
                                          adj_link,recvr_link)
        ...
        if do_FA:        
            if any(self.grid.at_node["water__unit_flux_in"]!=1):
                wg_q = self.grid.at_node["water__unit_flux_in"]*self.grid.dx*self.grid.dx
                # Only core nodes (status == 0) need to receive a weight
                wg_q[np.nonzero(self._grid.status_at_node)] = 0
                dis = wg_q
            else:
                dis = np.zeros(self.grid.number_of_nodes, dtype=np.int64) + self._node_cell_area
            
            da = np.zeros(self.grid.number_of_nodes, dtype=np.int64) + self._node_cell_area            
    ...
    def removeDepressions(self):
        ...
        self._sort[:]= np.argsort(np.array(self._deprFreeDEM.reshape(self.grid.number_of_nodes)))
    ...
    def accumulate_flow_RD(self, props_Pf,hillFlow=False):    
        ...
        a[:]= np.array(rd.FlowAccumFromProps(props=props_Pf,weights=wg).reshape(self.grid.number_of_nodes))
    ...
    def update_Hill_FDFA(self, updateDepressions=False):      
        if not self._seperate_Hill_Flow:
            raise ValueError("If hillslope properties are updated, the seperate_Hill_Flow property of the FlowDirAccPf class should be True upon initialisation")
        if self._suppress_out:
            with suppress_stdout_stderr():
                sys.stdout = open(os.devnull, "w")
                sys.stderr = open(os.devnull, "w")
                self.calculateFlowDir_Acc(hillFlow=True, updateDepressions=updateDepressions)
                sys.stdout = sys.__stdout__
                sys.stderr = sys.__stderr__
        else:
             self.calculateFlowDir_Acc(hillFlow=True, updateDepressions=updateDepressions)
    ...
    def run_one_step(self):
        if self._suppress_out:
            with suppress_stdout_stderr():
                sys.stdout = open(os.devnull, "w")
                sys.stderr = open(os.devnull, "w")
                self.calculateFlowDir_Acc(hillFlow=False, updateDepressions=self._updateFlowDepressions)
                if self._seperate_Hill_Flow:
                    if self._update_HillFlow_instanteneous:
                        self.calculateFlowDir_Acc(hillFlow=True, updateDepressions=self._updateHillDepressions)
