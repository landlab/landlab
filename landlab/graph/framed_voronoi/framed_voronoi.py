...
class HorizontalRectVoronoiGraph:
    ...
    @staticmethod
    def xy_of_node(shape, xy_spacing=(1.0, 1.0), xy_of_lower_left=(0.0, 0.0), 
        xy_min_spacing=(0.5, 0.5), random_seed=True, seed=(200, 500)):
        ...
        --------
        >>> from landlab.graph.framed_voronoi.framed_voronoi import HorizontalRectVoronoiGraph
        >>> HorizontalRectVoronoiGraph.xy_of_node((3, 3), random_seed=False, seed=(200, 500))[0][3:6]       # doctest: +NORMALIZE_WHITESPACE
        array([ 0.        ,  1.07341707,  2.        ])
        >>> HorizontalRectVoronoiGraph.xy_of_node((3, 3), random_seed=False, seed=(200, 500))[1][3:6]       # doctest: +NORMALIZE_WHITESPACE
        array([ 0.749     ,  1.03337157,  1.251     ])
        """
        
        n_rows, n_cols = shape
        max_move = [(xy_spacing[0] - xy_min_spacing[0]) / 2, (xy_spacing[1] - xy_min_spacing[1]) / 2]
        n_nodes = HorizontalRectVoronoiGraph.number_of_nodes(shape)
        n_core_nodes = n_nodes - HorizontalRectVoronoiGraph.number_of_perimeter_nodes(shape)
        ...
        if random_seed == True:
            ...
            xy_random_generator = (np.random.Generator(np.random.PCG64(seed[0])),
                np.random.Generator(np.random.PCG64(seed[1])))
        
        x_y_random_move = (xy_random_generator[0].random(n_core_nodes) * max_move[0] * 2 - max_move[0],
        ...
        for i in range (1, n_rows - 1):
            y_of_node[i, 0] -= max_move[1] + 1.e-3
            y_of_node[i, n_cols - 1] += max_move[1] + 1.e-3
...
class FramedVoronoiGraph(DelaunayGraph):
    ...
    >>> graph = FramedVoronoiGraph((3, 3), node_layout='rect', random_seed=False)
    ...
    def __init__(
        ...
        #####################################
        try:
            shape_ = np.asfarray(np.broadcast_to(shape, 2))
        ...
        try:
            ...
            self._xy_of_lower_left = (float(xy_of_lower_left_[0]), float(xy_of_lower_left_[1]))
        except TypeError:
            raise TypeError("xy of lower left must be a float or a tuple of floats")
            
        if node_layout not in ("rect"):
            raise ValueError("node_layout not understood")
        self._node_layout = "rect"
        if orientation not in ("horizontal"):
        ...
        try:
            ...
            self._xy_min_spacing = (float(xy_min_spacing_[0]), float(xy_min_spacing_[1]))
        except TypeError:
            raise TypeError("minimal spacing must be a float or a tuple of floats")
        try:
            self._random_seed = bool(random_seed)
        except TypeError:
            raise TypeError("random see must be True or False")
        try:
            seed_ = np.asfarray(np.broadcast_to(seed, 2))
            self._seed = (int(seed_[0]), int(seed_[1]))
        except TypeError:
            raise TypeError("seed must be an int or a tuple of ints")
            
        # 2. Construction of the layout and the x-y coordinates of nodes
        ################################################################
        x_of_node, y_of_node = layout.xy_of_node(
            self._shape, xy_spacing=self._xy_spacing, xy_of_lower_left=self._xy_of_lower_left,
            xy_min_spacing=self._xy_min_spacing, random_seed=self._random_seed, seed=self._seed)
