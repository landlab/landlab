    ...
    * cells: Voronoi polygons
                ...
                "node": xr.DataArray(
                    ...
                    coords={
                        "x_of_node": xr.DataArray(
                            self._delaunay["vertices"][:, 0], dims=("node",)
                        ),
                        "y_of_node": xr.DataArray(
                            self._delaunay["vertices"][:, 1], dims=("node",)
                        ),
                    },
                    dims=("node",),
                ),
                "corner": xr.DataArray(
                    ...
                    coords={
                        "x_of_corner": xr.DataArray(
                            self._voronoi["vertices"][:, 0], dims=("corner",)
                        ),
                        "y_of_corner": xr.DataArray(
                            self._voronoi["vertices"][:, 1], dims=("corner",)
                        ),
                    },
                    dims=("corner",),
                ),
    ...
    def _number_cells(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        nodes_at_cell = np.array(
            [
                idx
                for idx in range(len(self._delaunay["vertex_markers"]))
    ...
    def _get_faces_at_cell(self) -> np.ndarray:
        ...
        for cell in np.arange(len(nodes_at_cell)):
            faces[cell] = np.where(
                np.isin(self._delaunay["edges"], nodes_at_cell[cell])
            )[0]
