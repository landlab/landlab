...
class TriangleMesh:
    ...
    def write_poly_file(self):
        """Write a .poly file, to be read later by Triangle."""
        path_to_poly_file = './tmp.poly'
        number_of_vertices = self._vertices.shape[0]
        number_of_edges = self._segments.shape[0]

        first_header = [number_of_vertices, 2, 0, 0] # n_points, n_dims, n_attributes, y/n boundary markers
        second_header = [number_of_edges, 0] # n_edges, y/n boundary markers
        third_header = [0]

        # TODO refactor so we can have interior boundaries, attributes, boundary markers

    def _triangulate(self):
        """Call Triangle from the command line."""
        # TODO: this should replace self.triangulate()
        pass 

    def construct_mesh(self):
        """Construct a mesh from Triangle's output files."""
        pass
