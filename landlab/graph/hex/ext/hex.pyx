...
def fill_xy_of_node_rect_horizontal(
    ...
    # for offset in prange(0, n_nodes, stride, nogil=True, schedule="static"):
    #     row = (offset // stride) * 2
    #     for n in range(offset, offset + n_cols):
    #         x_of_node[n] = n - offset
    #         y_of_node[n] = row

    # for offset in prange(n_cols, n_nodes, stride, nogil=True, schedule="static"):
