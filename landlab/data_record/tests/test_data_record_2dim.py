...
def test_coordinates(dr_2dim):
    ...
    dr_2dim.add_record(time=[10., 20.], new_record={"new_data": ((), (120., 130.))})
...
def test_add_record(dr_2dim):
    ...
    dr_2dim.add_record(
        time=[20.], new_record={"mean_elevation": (["time"], np.array([130.]))}
    )
    assert (
    ...
    dr_2dim.add_record(
        time=[10.],
        item_id=[1],
        new_record={"size": (["item_id", "time"], np.array([[.3]]))},
    )
...
def test_get_data(dr_2dim):
    assert (
        dr_2dim.get_data(time=[0.], item_id=[1], data_variable="grid_element") == "link"
    )
...
def test_set_data(dr_2dim):
    ...
    dr_2dim.set_data(time=[0.], data_variable="mean_elevation", new_value=150.)
