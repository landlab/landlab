...
def set_flux_coefficients( mg, dx ):
	...
	#self.K = Kmax * ( ( ( self.dx*(self.nr - 1) ) - yf) / (self.dx*self.nr) )**Kexp  # increases downward
	#self.K = Kmax * ( (yf-self.dx/2) / (self.dx*(self.nr-2)) )**Kexp  # increases upward
	#self.K = Kmax * ( ( self.dx*(self.nc - 2) - (xf-self.dx/2) ) / (self.dx*(self.nc-2)) )**Kexp  # increases to left
	#self.K = Kmax * ( (xf-self.dx/2) / (self.dx*(self.nc-2)) )**Kexp  # increases to right
